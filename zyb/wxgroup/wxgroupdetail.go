...
func GetGroupDetailInfo() {
	InitFileInfo()
	trunks := splitSlice(GetGroupDetailGroupIds, 70000)
	fmt.Println("分片数量：", len(trunks), " 已导出数量:", fileItemCnt, " 剩余未导出数量：", len(GetGroupDetailGroupIds))

	if fileItemCnt == 0 {
		appendRecord(fileName, []string{"群ID", "群名称", "主体名称", "建群时间", "在群人数", "群活动", "四级分类", "五级分类", "六级分类", "所属战队分组", "master", "slave", "slave个数", "master状态", "slave状态", "群主角色"})
	}

	// fmt.Println("群ID,群名称,主体名称,建群时间,在群人数,群活动,四级分类,五级分类,六级分类,所属战队分组,master,slave,slave个数,master状态,slave状态,群主角色,")
...
func getGroupMemberDetail(wxGroupId string) (detail MemberDetail, err error) {
	...
	for _, item := range respData.Data.List {
		...
		if wxGroupDetail.WxGroupId != "" {
			...
			detail.CreateDate = wxGroupDetail.CreateTime
